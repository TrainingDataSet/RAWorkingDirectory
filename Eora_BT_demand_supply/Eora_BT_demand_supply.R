...
# 2024-03-24: Introduction of parallel programming [running time about 98 min -> about 15 min per year (653% improvement)]
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    write_csv(BT_EachYear4, paste0("Eora_BT_demand_supply/Eora_BT_demand_supply_",
                                   year_lst[1], "-", year_lst[3], "_sample.csv"))
  }
  
}

clusterExport(cl, varlist = c("task", "year_lst", "country_lst", "country_lst2", 
                              "industry_lst", "industry_lst2", "cal_BT_demand_supply", 
                              "read_csv", "cal_beta", "cal_alpha", "%>%", 
                              "bind_rows", "bind_cols", "arrange", "write_csv"))
...
# [33years, 2 countries, 2 industries] 25 mins -> 5 mins (500% improvement)
# [33years, 5 countries, 2 industries] 98 mins -> 15 mins (653% improvement)
time <- Sys.time()
