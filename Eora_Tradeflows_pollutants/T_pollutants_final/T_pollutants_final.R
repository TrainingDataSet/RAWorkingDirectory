...
# 2025-04-29: Convert "Pollutants_weight.csv" into "Pollutants_weight_countryLevel.csv"
...
#   pollutants_df: Pollutants_weight/Pollutants_weight_countryLevel.csv
...
cal_total_trade_flows <- function(exports_df, pollutants_df, year, country_i, country_j) {
  
  
  #----------------- Vector of Pollutants Weight -----------------#
  
  CO2_is <- as.vector(pollutants_df[(pollutants_df$Year == year) &
                                      (pollutants_df$Country == country_i), "CO2_rate"])[[1]]
  
  CO2_ju <- as.vector(pollutants_df[(pollutants_df$Year == year) &
                                      (pollutants_df$Country == country_j), "CO2_rate"])[[1]]
  
  GHG_is <- as.vector(pollutants_df[(pollutants_df$Year == year) &
                                      (pollutants_df$Country == country_i), "GHG_rate"])[[1]]
  
  GHG_ju <- as.vector(pollutants_df[(pollutants_df$Year == year) &
                                      (pollutants_df$Country == country_j), "GHG_rate"])[[1]]
  ...
  EX_ij <- as.vector(exports_df[(exports_df$Year == year) &
                                  ...
                                  (exports_df$Country_j == country_j), "Exports"])[[1]]
  
  EX_ji <- as.vector(exports_df[(exports_df$Year == year) &
                                  (exports_df$Country_i == country_j) &
                                  (exports_df$Country_j == country_i), "Exports"])[[1]]
  ...
  T_CO2 <- (T_CO2_ij + T_CO2_ji) * 26
  T_GHG <- (T_GHG_ij + T_GHG_ji) * 26
...
# exports_df <- read_csv("Eora_final_exports_pair_FromIndustryToCountry/Eora_final_exports.csv")
# pollutants_df <- read_csv("Pollutants_weight/Pollutants_weight_countryLevel.csv")
# year <- 2022
# country_i <- "VNM"
# country_j <- "VEN"
# 
# print(cal_total_trade_flows(exports_df, pollutants_df, year, country_i, country_j))
# 
# # Running Time of Function
# microbenchmark(
#   test <- cal_total_trade_flows(exports_df, pollutants_df, year, country_i, country_j),
#   times <- 1000
# )  # --> 0.9 microsec
...
task <- function(year_lst) {
  ...
  T_lst <- list("Year" = lst_length,
                ...
                "T_CO2_final" = lst_length,
                "T_GHG_final" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        T_var <- cal_total_trade_flows(exports_df, pollutants_df, year, i, j)
        T_CO2 <- T_var[1]
        T_GHG <- T_var[2]
        ...
        T_lst$T_CO2_final[index] <- T_CO2
        T_lst$T_GHG_final[index] <- T_GHG
    ...
    write_csv(T_EachYear4, paste0("T_pollutants_final/T_pollutants_", 
