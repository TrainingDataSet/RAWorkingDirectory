...
# exports_df <- read_csv("Eora_total_exports_pair_FromIndustryToCountry/Eora_total_exports.csv")
...
task <- function(year_lst) {
  ...
  T_lst <- list("Year" = lst_length,
                ...
                "T_CO2_total" = lst_length,
                "T_GHG_total" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        T_lst$T_CO2_total[index] <- T_CO2
        T_lst$T_GHG_total[index] <- T_GHG
    ...
    write_csv(T_EachYear4, paste0("T_pollutants_total/T_pollutants_", 
