...
# 2025-04-29: Convert final exports to intermediate exports
...
cal_exports <- function(df, country_i, country_j) {
  ...
  export <- df[df$X2 == country_i, which(df[3, ] == country_j & grepl("^c", df[4, ]))]
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    # save as a csv file
    write_csv(trade_flows_df, paste0("Eora_inter_exports_pair_FromIndustryToCountry/Eora_exports_", 
