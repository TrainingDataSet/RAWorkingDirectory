...
cal_exports <- function(df, country_i, country_j, from_industry) {
  
  export <- df[(df$X2 == country_i) & (df$X3 == from_industry), which((df[3, ] == country_j))]
  export <- df[(df$X2 == country_i) & (df$X3 == from_industry), which((df[3, ] == country_j))]
...
# from_industry <- "c15"
# 
# test <- cal_exports(df, country_i, country_j, from_industry)
# print(test)
# 
# 
...
# for (i in 1:100) {
#   time <- Sys.time()
#   test2 <- cal_exports(df, "DZA", "AGO", "c1")
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst) {
      for (j in country_lst[-num]) {
        for (k in industry_lst) {
          exports <- cal_exports(df, i, j, k)
    ...
    # save as a csv file
    write_csv(trade_flows_df, paste0("Eora_exports_pair_FromIndustryToCountry/Eora_exports_", 
