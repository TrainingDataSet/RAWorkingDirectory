...
cal_export_import_sales_va <- function(df, country) {
  ...
  sales <- as.numeric(sales_vad[1, ])
  vad_cur <- as.numeric(sales_vad[2, ])
  
  return (list("exports_sum" = exports_sum, "imports_sum" = imports_sum, "sales" = sales, "vad_cur" = vad_cur))
}
...
# df <- read_csv("Eora_csv_files/Eora2012.csv")
# country = "TUN"
...
task <- function(year_lst) {
  ...
  lst <- list("Year" = lst_length, "Country_i" = lst_length, "Industry_s" = lst_length, 
              "Export" = lst_length, "Import" = lst_length, "Sales" = lst_length, "VA_cur" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (country_i in country_lst) {
      ...
      lst$Sales[index:(index+25)] <- val$sales
    ...
    # save as a csv file
    write_csv(df, paste0("Export_Import_Sales_VA/Export_Import_Sales_VA_EntireCountry", 
