...
# 2024-08-05: Calculate LB considering nothing [Sector(x), Pollutants(x)]
...
#
# Returns:
#   LB: LB variable considering nothing [Sector(x), Pollutants(x)]
#---------------------------------
...
cal_LB <- function(df, year, country_i, country_j, oecd) {
  ...
  considering_oecd_i <- except_j[!except_j$Country_j %in% oecd, "Trade_flows"]
  
  sum_exj <- sum(except_j["Trade_flows"])
  
  lhs <- as.vector(considering_oecd_i / sum_exj)[[1]]
  ...
  considering_oecd_j <- except_i[!except_i$Country_j %in% oecd, "Trade_flows"]
  
  sum_exi <- sum(except_i["Trade_flows"])
  
  rhs <- as.vector(considering_oecd_j / sum_exi)[[1]]
  
  
  #----------------- Final Result of LB_network_1st  -----------------#
  
  LB <- as.numeric(1 - (sum(abs(lhs - rhs)) / 2))
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    write_csv(LB_EachYear3, paste0("consideringNonOECD/LB/LB_nonoecd_", 
