...
cal_MB <- function(df, year, country_i, country_j, oecd) {
  ...
  considering_nonoecd_i <- except_j[!except_j$Country_j %in% oecd, "Trade_flows"]
  
  sum_exj <- sum(except_j["Trade_flows"])
  
  lhs <- as.vector(considering_nonoecd_i / sum_exj)[[1]]
  ...
  considering_nonoecd_j <- except_i[!except_i$Country_j %in% oecd, "Trade_flows"]
  
  sum_exi <- sum(except_i["Trade_flows"])
  
  rhs <- as.vector(considering_nonoecd_j / sum_exi)[[1]]
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    write_csv(MB_EachYear3, paste0("consideringNonOECD/MB/MB_nonoecd_", 
