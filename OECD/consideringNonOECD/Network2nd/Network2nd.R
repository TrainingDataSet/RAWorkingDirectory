...
#   MB_df: consideringOECD/MB/MB.csv
#   LB_df: consideringOECD/LB/LB.csv
#   year: Year
...
cal_network2nd <- function(df, MB_df, LB_df, year, country_i, country_j, oecd) {
  ...
  sum_country_i_thirdCountry <- sum(country_i_thirdCountry[, "Trade_flows"])
  
  # Considering only Non-OECD
  country_i_thirdCountry <- country_i_thirdCountry[!country_i_thirdCountry$Country_j %in% oecd, ]
  
  country_i_thirdCountry[, "Trade_flows"] <- sapply(country_i_thirdCountry[, "Trade_flows"], function(x) (x / sum_country_i_thirdCountry))
  ...
  sum_country_j_thirdCountry <- sum(country_j_thirdCountry[, "Trade_flows"])
  
  # Considering only Non-OECD
  country_j_thirdCountry <- country_j_thirdCountry[!country_j_thirdCountry$Country_j %in% oecd, ]
  
  country_j_thirdCountry[, "Trade_flows"] <- sapply(country_j_thirdCountry[, "Trade_flows"], function(x) (x / sum_country_j_thirdCountry))
  ...
  network2nd_MB <- sum(merging2[, "Trade_flows.x"] * merging2[, "Trade_flows.y"] * merging2[, "MB_nonoecd"])
  network2nd_LB <- sum(merging2[, "Trade_flows.x"] * merging2[, "Trade_flows.y"] * merging2[, "LB_nonoecd"])
...
# MB_df <- read_csv("consideringNonOECD/MB/MB_nonoecd.csv")
# LB_df <- read_csv("consideringNonOECD/LB/LB_nonoecd.csv")
# year <- 2022
# country_i <- "KOR"
# country_j <- "JPN"
...
task <- function(year_lst) {
  ...
  Network2nd_lst <- list("Year" = lst_length,
                         ...
                         "MB_network2nd_nonoecd" = lst_length,
                         "LB_network2nd_nonoecd" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        Network2nd_lst$MB_network2nd_nonoecd[index] <- MB
        Network2nd_lst$LB_network2nd_nonoecd[index] <- LB
    ...
    write_csv(Network2nd_EachYear3, paste0("consideringNonOECD/Network2nd/Network2nd_nonoecd_", 
...
clusterExport(cl, varlist = c("task", "year_lst", "country_lst", "country_lst2",
                              ...
                              "read_csv", "%>%", "arrange", "write_csv", "df", "oecd"))
