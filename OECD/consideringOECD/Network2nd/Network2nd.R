...
cal_network2nd <- function(df, MB_df, LB_df, year, country_i, country_j, oecd) {
  ...
  network2nd_MB <- sum(merging2[, "Trade_flows.x"] * merging2[, "Trade_flows.y"] * merging2[, "MB_oecd"])
  network2nd_LB <- sum(merging2[, "Trade_flows.x"] * merging2[, "Trade_flows.y"] * merging2[, "LB_oecd"])
...
MB_df <- read_csv("consideringOECD/MB/MB_oecd.csv")
LB_df <- read_csv("consideringOECD/LB/LB_oecd.csv")
year <- 2022
country_i <- "KOR"
country_j <- "JPN"
...
task <- function(year_lst) {
  ...
  Network2nd_lst <- list("Year" = lst_length,
                         ...
                         "MB_network2nd_oecd" = lst_length,
                         "LB_network2nd_oecd" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        Network2nd_lst$MB_network2nd_oecd[index] <- MB
        Network2nd_lst$LB_network2nd_oecd[index] <- LB
    ...
    write_csv(Network2nd_EachYear3, paste0("consideringOECD/Network2nd/Network2nd_oecd_", 
