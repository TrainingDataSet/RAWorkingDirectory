...
task <- function(year_lst) {
  ...
  LB_lst <- list("Year" = lst_length,
                 ...
                 "LB_CO2_nonoecd" = lst_length,
                 "LB_GHG_nonoecd" = lst_length
  )
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        LB_lst$LB_CO2_nonoecd[index] <- LB_CO2
        LB_lst$LB_GHG_nonoecd[index] <- LB_GHG
    ...
    write_csv(LB_EachYear3, paste0("consideringPollutantsNonOECD/LB/LB_pollutants_nonoecd", 
