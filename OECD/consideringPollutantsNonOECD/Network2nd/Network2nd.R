...
cal_network2nd <- function(df, MB_df, LB_df, year, country_i, country_j, oecd) {
  ...
  sum_country_i_thirdCountry <- as.numeric(colSums(country_i_thirdCountry[c("T_CO2", "T_GHG")]))
  
  # Considering only OECD
  country_i_thirdCountry <- country_i_thirdCountry[!country_i_thirdCountry$Country_j %in% oecd, ]
  
  country_i_thirdCountry[c("T_CO2", "T_GHG")] <- sweep(country_i_thirdCountry[c("T_CO2", "T_GHG")], 2, sum_country_i_thirdCountry, "/")
  ...
  sum_country_j_thirdCountry <- as.numeric(colSums(country_j_thirdCountry[c("T_CO2", "T_GHG")]))
  
  # Considering only OECD
  country_j_thirdCountry <- country_j_thirdCountry[!country_j_thirdCountry$Country_j %in% oecd, ]
  
  country_j_thirdCountry[c("T_CO2", "T_GHG")] <- sweep(country_j_thirdCountry[c("T_CO2", "T_GHG")], 2, sum_country_j_thirdCountry, "/")
  ...
  network2nd_MB <- colSums(merging2[, c("T_CO2.x", "T_GHG.x")] * merging2[, c("T_CO2.y", "T_GHG.y")] * merging2[, c("MB_CO2_nonoecd", "MB_GHG_nonoecd")])
  network2nd_LB <- colSums(merging2[, c("T_CO2.x", "T_GHG.x")] * merging2[, c("T_CO2.y", "T_GHG.y")] * merging2[, c("LB_CO2_nonoecd", "LB_GHG_nonoecd")])
...
# MB_df <- read_csv("consideringPollutantsNonOECD/MB/MB_pollutants_nonoecd.csv")
# LB_df <- read_csv("consideringPollutantsNonOECD/LB/LB_pollutants_nonoecd.csv")
# year <- 2022
# country_i <- "KOR"
# country_j <- "JPN"
...
# 
# print(cal_network2nd(df, MB_df, LB_df, year, country_i, country_j, oecd))
# 
# # Running Time of Function
# microbenchmark(
#   test = cal_network2nd(df, MB_df, LB_df, year, country_i, country_j, oecd),
#   times = 1000
# ) # --> 0.09 sec
...
task <- function(year_lst) {
  ...
  Network2nd_lst <- list("Year" = lst_length,
                         ...
                         "MB_network2nd_CO2_nonoecd" = lst_length,
                         "MB_network2nd_GHG_nonoecd" = lst_length,
                         "LB_network2nd_CO2_nonoecd" = lst_length,
                         "LB_network2nd_GHG_nonoecd" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        Network2nd <- cal_network2nd(df, MB_df, LB_df, year, i, j, oecd)
        MB_CO2 <- Network2nd[1]
        MB_GHG <- Network2nd[2]
        LB_CO2 <- Network2nd[3]
        LB_GHG <- Network2nd[4]
        ...
        Network2nd_lst$MB_network2nd_CO2_nonoecd[index] <- MB_CO2
        Network2nd_lst$MB_network2nd_GHG_nonoecd[index] <- MB_GHG
        Network2nd_lst$LB_network2nd_CO2_nonoecd[index] <- LB_CO2
        Network2nd_lst$LB_network2nd_GHG_nonoecd[index] <- LB_GHG
    ...
    write_csv(Network2nd_EachYear3, paste0("consideringPollutantsNonOECD/Network2nd/Network2nd_pollutants_nonoecd", 
