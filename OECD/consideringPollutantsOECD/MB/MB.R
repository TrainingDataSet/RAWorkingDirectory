...
task <- function(year_lst) {
  ...
  MB_lst <- list("Year" = lst_length,
                 ...
                 "MB_CO2_oecd" = lst_length,
                 "MB_GHG_oecd" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        MB_lst$MB_CO2_oecd[index] <- MB_CO2
        MB_lst$MB_GHG_oecd[index] <- MB_GHG
    ...
    write_csv(MB_EachYear3, paste0("consideringPollutantsOECD/MB/MB_pollutants_oecd", 
