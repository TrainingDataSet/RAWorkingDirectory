...
cal_network2nd <- function(df, MB_df, LB_df, year, country_i, country_j, oecd) {
  ...
  network2nd_MB <- colSums(merging2[, c("T_CO2.x", "T_GHG.x")] * merging2[, c("T_CO2.y", "T_GHG.y")] * merging2[, c("MB_CO2_oecd", "MB_GHG_oecd")])
  network2nd_LB <- colSums(merging2[, c("T_CO2.x", "T_GHG.x")] * merging2[, c("T_CO2.y", "T_GHG.y")] * merging2[, c("LB_CO2_oecd", "LB_GHG_oecd")])
...
# MB_df <- read_csv("consideringPollutantsOECD/MB/MB_pollutants_oecd.csv")
# LB_df <- read_csv("consideringPollutantsOECD/LB/LB_pollutants_oecd.csv")
# year <- 2022
# country_i <- "KOR"
# country_j <- "JPN"
...
task <- function(year_lst) {
  ...
  Network2nd_lst <- list("Year" = lst_length,
                         ...
                         "MB_network2nd_CO2_oecd" = lst_length,
                         "MB_network2nd_GHG_oecd" = lst_length,
                         "LB_network2nd_CO2_oecd" = lst_length,
                         "LB_network2nd_GHG_oecd" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        Network2nd_lst$MB_network2nd_CO2_oecd[index] <- MB_CO2
        Network2nd_lst$MB_network2nd_GHG_oecd[index] <- MB_GHG
        Network2nd_lst$LB_network2nd_CO2_oecd[index] <- LB_CO2
        Network2nd_lst$LB_network2nd_GHG_oecd[index] <- LB_GHG
    ...
    write_csv(Network2nd_EachYear3, paste0("consideringPollutantsOECD/Network2nd/Network2nd_pollutants_oecd", 
