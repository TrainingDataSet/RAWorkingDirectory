...
cal_corr_gdp_pollutants <- function(owid, t, country_i, country_j) {
  
  #-------------- Extract Vector of GDP & Pollutants in Time Window --------------#
  
  rgdp_i <- owid[(owid$Year %in% t) &
                (owid$Country == country_i), "rgdp"]
  
  co2_i <- owid[(owid$Year %in% t) &
                (owid$Country == country_i), "co2"]
  
  rgdp_j <- owid[(owid$Year %in% t) &
                 (owid$Country == country_j), "rgdp"]
  
  co2_j <- owid[(owid$Year %in% t) &
...
# Time window list
tw_lst <- list('1' = c(2007:2013),
               '2' = c(2014:2020))

# Create List
lst_length <- numeric(length(tw_lst) * length(country_lst) * length(country_lst))
...
for (t in c("1", "2")) {
  for (i in country_lst) {
    for (j in country_lst2) {
      Corr_GDP_pollutants <- cal_corr_gdp_pollutants(owid, tw_lst[[t]], i, j)
  ...
  write_csv(Corr_df, paste0("OWID/OWID_corr.csv"))
}
