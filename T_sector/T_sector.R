...
# 2024-08-05: Calculate total trade flows considering sector
...
#---------------------------------
# Calculate total trade flows considering sector
#
# Args:
...
#
# Returns:
#   Total_trade_flows: Total trade flows considering sector
#---------------------------------

cal_total_trade_flows <- function(df, year, country_i, country_j, sector_s, sector_u) {
  
  #------------ Sum of Exports of Bilateral Countries ------------#
  
  EX_ij <- as.numeric(df[(df$Year == year) &
                         (df$Country_i == country_i) &
                         (df$Sector_s == sector_s) &
                         (df$Country_j == country_j) &
                         (df$Sector_u == sector_u), "Exports"])
  
  EX_ji <- as.numeric(df[(df$Year == year) &
                         (df$Country_i == country_j) &
                         (df$Sector_s == sector_u) &
                         (df$Country_j == country_i) &
                         (df$Sector_u == sector_s), "Exports"])
...
# sector_u <- "services"
# 
# print(cal_total_trade_flows(df, year, country_i, country_j, sector_s, sector_u))
# 
# # Running Time of Function
# microbenchmark(
#   test = cal_total_trade_flows(df, year, country_i, country_j, sector_s, sector_u),
#   times = 100
# ) # --> 0.08 sec
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
      ...
      for (j in country_lst2[num:length(country_lst2)]) {
        for (s in sector_lst) {
          for (u in sector_lst2) {
            Total_trade_flows <- cal_total_trade_flows(df, year, i, j, s, u)
    ...
    T_EachYear3$Sector_s <- factor(T_EachYear3$Sector_s, levels = sector_lst)
    T_EachYear3$Sector_u <- factor(T_EachYear3$Sector_u, levels = sector_lst)
    
    T_EachYear4 <- T_EachYear3 %>%
      arrange(Year, Country_i, Country_j, Sector_s, Sector_u)
    
    # Save it as a csv file
    write_csv(T_EachYear4, paste0("T_sector/T_sector_", 
                                   year_lst[1], "-", year_lst[3], ".csv"))
  }
}

clusterExport(cl, varlist = c("task", "year_lst", "country_lst", "country_lst2",
                              "sector_lst", "sector_lst2", "cal_total_trade_flows",
