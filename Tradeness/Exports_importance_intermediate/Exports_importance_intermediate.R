...
# 2024-01-19: Considering Intermediate goods exports only (Not final goods)
...
cal_tradeness <- function(df, country_i, country_j) {
  ...
  export <- df[which(df$X2 == country_i), which(df[3, ] == country_j & grepl("^c", df[4, ]))]
...
task <- function(year_lst) {
  ...
  Tradeness_lst <- list("Year" = lst_length, "Country_i" = lst_length, "Country_j" = lst_length,
                        "Industry_s" = lst_length, "Exports_importance_intermediate" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (country_i in country_lst) {
      for (country_j in country_lst2) {
        ...
        Tradeness_lst$Exports_importance_intermediate[index:(index+25)] <- tradeness
        index <- index + 26
      }
    }
    ...
    # save as a csv file
    write_csv(Tradeness_df, paste0("Tradeness/Exports_importance_intermediate/Exports_importance_intermediate_", 
