...
# 2024-10-31: Imports of industry s of country i from foreign country j
...
### Imports of industry s of country i from foreign country j ###

#---------------------------------
# Calculate Imports of industry s of country i from foreign country j
#
# Args:
...
#
# Returns:
#   Imports_importance: Imports of industry s of country i from foreign country j (ratio)
#---------------------------------

cal_imports_imp <- function(df, country_i, country_j) {
  
  import <- df[which(df$X2 == country_j), which(df[3, ] == country_i & grepl("^c", df[4, ]))]
  import_sum <- colSums(as.data.frame(lapply(import, as.numeric)))
  imports_imp <- import_sum / sum(import_sum)
...
df <- read_csv("Eora_csv_files/Eora2020.csv")
country_i <- "CAN"
country_j <- "UKR"

print(cal_imports_imp(df, country_i, country_j))
# 
# # Running Time of Function
# microbenchmark(
#   test = cal_imports_imp(df, country_i, country_j),
#   times = 1000
# ) # --> 0.09 sec
...
task <- function(year_lst) {
  ...
  Imports_imp_lst <- list("Year" = lst_length, "Country_i" = lst_length, "Country_j" = lst_length,
                        "Industry_s" = lst_length, "Imports_importance" = lst_length)
  ...
  for (year in year_lst) {
    ...
    # Calculate Imports importance with the function and append to list
    for (country_i in country_lst) {
      for (country_j in country_lst2) {
        if (country_i == country_j) 
          next
        imports_imp <- cal_imports_imp(df, country_i, country_j)
        Imports_imp_lst$Year[index:(index+25)] <- rep(year, 26)
        Imports_imp_lst$Country_i[index:(index+25)] <- rep(country_i, 26)
        Imports_imp_lst$Country_j[index:(index+25)] <- rep(country_j, 26)
        Imports_imp_lst$Industry_s[index:(index+25)] <- industry_lst
        Imports_imp_lst$Imports_importance[index:(index+25)] <- imports_imp
        index <- index + 26
      }
    }
    
    # list -> DataFrame
    Imports_imp_df = data.frame(Imports_imp_lst)
    
    # save as a csv file
    write_csv(Imports_imp_df, paste0("Imports_importance/Imports_importance_", 
...
clusterExport(cl, varlist = c("task", "year_lst", "country_lst", "country_lst2", 
                              "industry_lst", "read_csv", "write_csv", "cal_imports_imp"))
