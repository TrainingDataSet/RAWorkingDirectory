...
# country_j <- "VEN"
# 
# print(cal_LB(df, year, country_i, country_j))
# 
# # Running Time of Function
# microbenchmark(
#   test = cal_LB(df, year, country_i, country_j),
#   times = 100
# )  # --> 0.01 sec
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    write_csv(LB_EachYear3, paste0("consideringNothing/LB/LB_", 
                                   year_lst[1], "-", year_lst[3], ".csv"))
  }
}

clusterExport(cl, varlist = c("task", "year_lst", "country_lst", "country_lst2",
                              "cal_LB", "read_csv", "%>%", "arrange", "write_csv", "df"))
