...
# 2024-08-15: Calculate BT considering pollutants
...
#---------------------------------
# Calculate BT considering pollutants
#
# Args:
#   t_df: T_pollutants/T_pollutants.csv
#   gdp_df: GDP/GDP.csv
#   year: Year
...
#
# Returns:
#   BT: BT variable considering pollutants
#---------------------------------
...
cal_BT <- function(t_df, gdp_df, year, country_i, country_j) {
  ...
  Trade_flows <- t_df[(t_df$Year == year) &
                        (t_df$Country_i == country_i) &
                        (t_df$Country_j == country_j), ]
  
  T_CO2 <- Trade_flows$T_CO2_final
  T_GHG <- Trade_flows$T_GHG_final
...
# t_df <- read_csv("T_pollutants_final/T_pollutants_final.csv")
...
task <- function(year_lst) {
  ...
  BT_lst <- list("Year" = lst_length,
                 ...
                 "BT_CO2_final" = lst_length,
                 "BT_GHG_final" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        BT_lst$BT_CO2_final[index] <- BT_CO2
        BT_lst$BT_GHG_final[index] <- BT_GHG
    ...
    write_csv(BT_EachYear3, paste0("consideringPollutants/BT_final/BT_", 
...
stopCluster(cl)
print(Sys.time()-time)  # --> running time 7.747 mins
