...
cal_BT <- function(t_df, gdp_df, year, country_i, country_j) {
  ...
  T_CO2 <- Trade_flows$T_CO2
  T_GHG <- Trade_flows$T_GHG
...
# t_df <- read_csv("T_pollutants_inter/T_pollutants_inter.csv")
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    write_csv(BT_EachYear3, paste0("consideringPollutants/BT_new/BT_", 
