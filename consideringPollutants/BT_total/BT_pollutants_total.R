...
cal_BT <- function(t_df, gdp_df, year, country_i, country_j) {
  ...
  T_CO2 <- Trade_flows$T_CO2_total
  T_GHG <- Trade_flows$T_GHG_total
...
# t_df <- read_csv("T_pollutants_total/T_pollutants_total.csv")
...
task <- function(year_lst) {
  ...
  BT_lst <- list("Year" = lst_length,
                 ...
                 "BT_CO2_total" = lst_length,
                 "BT_GHG_total" = lst_length)
  ...
  for (year in year_lst) {
    ...
    for (i in country_lst[1:length(country_lst)-1]) {
      for (j in country_lst2[num:length(country_lst2)]) {
        ...
        BT_lst$BT_CO2_total[index] <- BT_CO2
        BT_lst$BT_GHG_total[index] <- BT_GHG
    ...
    write_csv(BT_EachYear3, paste0("consideringPollutants/BT_total/BT_", 
