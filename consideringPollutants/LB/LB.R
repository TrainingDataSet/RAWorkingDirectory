...
cal_LB <- function(df, year, country_i, country_j) {
  ...
  except_j <- except_j %>%
    group_by(Country_j) %>%
    summarise(T_CO2 = sum(T_CO2), T_GHG = sum(T_GHG)) %>%
    select(-Country_j)
  
  lhs <- sapply(except_j, function(x) (x / sum(x)))
  ...
  rhs <- sapply(except_i, function(x) (x / sum(x)))
...
task <- function(year_lst) {
  ...
  for (year in year_lst) {
    ...
    write_csv(LB_EachYear3, paste0("consideringPollutants/LB/LB_", 
