...
# 2024-08-18: Use pollutants data not considering sector
...
#
# Args:
#   df: Sum_pollutants/Sum_pollutants_HP_filtered_noSector.csv
#   t: Time Window
#   country_i: Code of country_s
#   country_j: Code of country_r
#
# Returns:
#   Corr_CO2: Correlation of CO2 
#   Corr_GHG: Correlation of GHG 
#---------------------------------

cal_corr <- function(df, t, country_i, country_j) {
...
# df <- read_csv("Sum_pollutants/Sum_pollutants_HP_filtered_noSector.csv")
# 
# t <- c(2010:2022)
# country_i <- "KOR"
# country_j <- "CHN"
# 
# print(cal_corr(df, t, country_i, country_j))
# 
# # Running Time of Function
# microbenchmark(
#   test = cal_corr(df, t, country_i, country_j),
#   times = 1000
# ) # --> 0.0006 sec
...
for (t in c("1", "2", "3", "4")) {
  ...
  for (i in country_lst[1:length(country_lst)-1]) {
    for (j in country_lst2[num:length(country_lst2)]) {
      pollutants <- cal_corr(df, tw_lst[[t]], i, j)
  ...
  write_csv(Corr_df3, paste0("consideringPollutants/Time_window/Corr_pollutants.csv"))
}
