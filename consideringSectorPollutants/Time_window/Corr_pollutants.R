...
# 2024-08-02: Apply HP_filter to Pollutants
...
#
# Args:
#   df: Sum_pollutants/Sum_pollutants_HP_filtered.csv
#   t: Time Window
...
#
# Returns:
#   Corr_CO2: Correlation of CO2 in (country_i, sector_s) and (country_j, sector_u)
#   Corr_GHG: Correlation of GHG in (country_i, sector_s) and (country_j, sector_u)
#---------------------------------

cal_corr <- function(df, t, country_i, country_j, sector_s, sector_u) {
  
  #-------------- Extract Vector of Pollutants in Time Window --------------#
  ...
  #------------------- Calculate Correlation of CO2 & GHG -------------------#
  
  Corr_CO2 <- cor(CO2_is, CO2_ju)
  Corr_GHG <- cor(GHG_is, GHG_ju)
  
  
  return (c(Corr_CO2, Corr_GHG))
...
# df <- read_csv("Sum_pollutants/Sum_pollutants_HP_filtered.csv")
# 
# t <- c(2010:2019)
# country_i <- "RUS"
# country_j <- "PRY"
...
# sector_u <- "goods"
# 
# print(cal_corr(df, t, country_i, country_j, sector_s, sector_u))
# 
...
# for (i in 1:100) {
#   time <- Sys.time()
#   test <- cal_corr(df, t, country_i, country_j, sector_s, sector_u)
#   time_vec[index] <- Sys.time() - time
#   index <- index + 1
# }
# print(mean(time_vec)) # --> 0.001 sec
...
for (t in c("1", "2", "3", "4")) {
    ...
    for (j in country_lst2[num:length(country_lst2)]) {
      for (s in sector_lst) {
        for (u in sector_lst2) {
          pollutants <- cal_corr(df, tw_lst[[t]], i, j, s, u)
          Corr_CO2 <- pollutants[1]
          Corr_GHG <- pollutants[2]
          ...
          Corr_lst$Corr_CO2[index] <- Corr_CO2
          Corr_lst$Corr_GHG[index] <- Corr_GHG
  ...
  write_csv(Corr_df4, paste0("consideringSectorPollutants/Time_window/Corr_pollutants.csv"))
}

print(Sys.time()-time)  # --> running time 1.12 mins
