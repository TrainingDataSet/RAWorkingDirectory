...
#---------------------------------
# Calculate difference of correlation between value added and pollutants 
...
#
# Returns:
#   Corr_diff: Corr(VAD, pollutants) of Country_i - Corr(VAD, pollutants) of Country_j
#---------------------------------
...
cal_corr_diff <- function(vad, pollutants, t, country_i, country_j, sector_s, sector_u) {
  ...
  Corr_VAD_CO2_is <- cor(vad_is$VAD, pollutants_is$CO2)
  Corr_VAD_GHG_is <- cor(vad_is$VAD, pollutants_is$GHG)
  Corr_VAD_CO2_ju <- cor(vad_ju$VAD, pollutants_ju$CO2)
  Corr_VAD_GHG_ju <- cor(vad_ju$VAD, pollutants_ju$GHG)
  
  Corr_diff_CO2 <- Corr_VAD_CO2_is - Corr_VAD_CO2_ju
  Corr_diff_GHG <- Corr_VAD_GHG_is - Corr_VAD_GHG_ju
...
for (t in c("1", "2", "3", "4")) {
  ...
  write_csv(Corr_diff_df, paste0("consideringSectorPollutants/Time_window/Corr_diff2.csv"))
}

print(Sys.time()-time)  # --> running time 4.15 mins